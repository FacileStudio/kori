...
func TestEditorDefaultsToEmpty(t *testing.T) {
	...
	if config.PromptEditKey != "ctrl+g" {
		t.Errorf("prompt_edit_key = %q, want ctrl+g by default", config.PromptEditKey)
	}
}
...
func TestEditorPartialConfigFromFile(t *testing.T) {
	...
	if config.PromptEditKey != "ctrl+g" {
		t.Errorf("prompt_edit_key = %q, want ctrl+g when not set in file", config.PromptEditKey)
	}
}
