...
func TestEditorDefaultsToEmpty(t *testing.T) {
	...
	if config.PromptEditKey != "ctrl+o" {
		t.Errorf("prompt_edit_key = %q, want ctrl+o by default", config.PromptEditKey)
	}
}
...
func TestEditorPartialConfigFromFile(t *testing.T) {
	...
	if config.PromptEditKey != "ctrl+o" {
		t.Errorf("prompt_edit_key = %q, want ctrl+o when not set in file", config.PromptEditKey)
	}
}
